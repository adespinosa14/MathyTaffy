...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    @objc func makeQuestions (){
        ...
        let objectOneVelocity = SKAction.move(by: CGVector(dx: -2 * self.frame.width, dy: 0), duration: TimeInterval(self.frame.width / 80))
        ...
        objectOne.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: self.questionAnswer.size().width / 4, height: self.questionAnswer.size().height))
        ...
        objectTwo.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: self.questionAnswer.size().width / 4, height: self.questionAnswer.size().height))
        ...
        objectThree.position = CGPoint(x: self.frame.midX + backgroundImg.size().width / 2, y: self.frame.minY + 200)
        objectThree.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: self.questionAnswer.size().width / 4, height: self.questionAnswer.size().height))
        ...
        answerChoiceThree.position = CGPoint(x: self.frame.midX + backgroundImg.size().width / 2, y:  self.frame.minY + 200)
    ...
    func gameSetUp(){
      ...
      //  if(shop.shopSharedInstance.shopContent[0] == true){
        ...
        let backgroundMove = SKAction.move(by: CGVector(dx: -backgroundImg.size().width, dy: 0), duration: 5)
        ...
        groundTest.position = CGPoint(x: self.frame.midX, y: self.frame.minY)
        ...
        let roofTexture = SKTexture(imageNamed: "backgroundQuestion")
        roofTest = SKSpriteNode(texture: roofTexture)
        roofTest.position = CGPoint(x: self.frame.midX, y: self.frame.maxY + 15)
        roofTest.size.height = 10
        roofTest.size.width = 10000
        roofTest.zRotation = 0
        roofTest.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: roofTest.size.width, height: roofTest.size.height))
        roofTest.physicsBody!.isDynamic = false
        roofTest.physicsBody!.contactTestBitMask = ColliderType.bird.rawValue
        roofTest.physicsBody!.categoryBitMask = ColliderType.gap.rawValue
        roofTest.physicsBody!.collisionBitMask = ColliderType.gap.rawValue
        
        roofTest.zPosition = 4
        addChild(roofTest)
        // player
        ...
        playerIcon.position = CGPoint(x: self.frame.midX - 10, y: self.frame.midY)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if beforeGameStart == 1 {
            ...
            timer = Timer.scheduledTimer(timeInterval: 7, target: self, selector: #selector(self.makeQuestions), userInfo: nil, repeats: true)
        ...
        playerIcon.physicsBody!.velocity = CGVector(dx: 0, dy: 0)
            playerIcon.physicsBody!.applyImpulse(CGVector(dx: 0, dy: 95))
